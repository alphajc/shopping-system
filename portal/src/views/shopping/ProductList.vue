    ...
    >
      ...
      >
        <el-form-item label="购买数量" prop="count">
          <el-input type="number" min=1 :max="maxCount" v-model="orderForm.count"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    const checkMobile = (rule, value, callback) => value.match(/^1[3456789]\d{9}$/) ? callback() : callback(new Error('请输入 11 位有效手机号码'))
    const checkCount = (rule, value, callback) => value < 1 || value > this.maxCount ? callback(new Error(`count ~ [1, ${this.maxCount}]`)) : callback()
    return {
      q: "",
      product: [],
      dialogVisible: false,
